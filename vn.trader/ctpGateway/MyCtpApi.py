...
class Log():
    def __init__(self, logpath):
        self.filename = logpath + 'Trading' + datetime.now().strftime('%Y-%m-%d') + '.log'
    ...
    def info(self,msg):
        ...
        try:
            msg = datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ' [INFO] ' + msg
    ...
    def error(self,msg):
        self.fp = file(self.filename,'a+')
        try:
            msg = datetime.now().strftime('%Y-%m-%d %H:%M:%S') + ' [ERROR] ' + msg
            self.fp.write(msg.encode('utf-8') + '\n')
        finally:
            self.fp.close()
...
class CtpMdApi(MdApi):
    ...
    def __init__(self, gateway):
        ...
        self.userID = "074553"  # 账号
        self.password = "LHQFH2818"  # 密码
    ...
    def onRspError(self, error, n, last):
        """错误回报"""
        errorContent = 'ErrorID:' + str(error['ErrorID']) + ',' + error['ErrorMsg']
        self.log.error(errorContent)
    ...
    def onRspUserLogout(self, data, error, n, last):
        ...
        if error['ErrorID'] == 0:
            ...
            logContent = u'行情服务器登出成功'
            self.log.info(logContent)

        # 否则，推送错误信息
        else:
            logContent = u'行情服务器登出失败'
            self.log.info(logContent)
            errorContent = 'ErrorID:' + str(error['ErrorID']) + ',' + error['ErrorMsg']
            self.log.error(errorContent)
...
class CtpTdApi(TdApi):
    ...
    def __init__(self, gateway):
        ...
        self.password = "LHQFH2818"  # 密码
        self.brokerID = "9999"  # 经纪商代码
        self.address = "tcp://180.168.146.187:10000"  # 服务器地址
    ...
    def onRspUserLogout(self, data, error, n, last):
        ...
        if error['ErrorID'] == 0:
            ...
            logContent = u'交易服务器登出成功'
            self.log.info(logContent)

        # 否则，推送错误信息
        else:
            logContent = u'交易服务器登出失败'
            self.log.info(logContent)
            errorContent = 'ErrorID:' + str(error['ErrorID']) + ',' + error['ErrorMsg']
            self.log.error(errorContent)
